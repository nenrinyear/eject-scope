...
Windows のリムーバブルドライブを安全に取り外せないとき、使用中のプロセスを確認する .NET 8 のトレイ常駐ツールです。

## できること

- リムーバブルドライブ（例: `E:\`）を選んで Restart Manager による使用中プロセスの検出
...
Windows 上で .NET 8 SDK をインストールしてから実行します。
...
第一版は Windows Restart Manager API を利用します。これは安全で標準的なプロセス関連付けを取得できますが、API 自体は個々のハンドルの正確なファイルパスを返しません。そのため一覧の「使用中の範囲」は登録したドライブルートを示します。管理者権限時のハンドル列挙による個別パス補完は、将来の拡張ポイントです。
